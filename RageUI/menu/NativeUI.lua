...
function UIMenuHorizontalOneLineGridPanel.New(LeftText, RightText)
    _UIMenuHorizontalOneLineGridPanel = {
        ...
        Audio = {Slider = "CONTINUOUS_SLIDER", Library = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", Id = nil},
        ParentItem = nil,
        Text = {
    ...
    if Txt ~= "no" then 
        ...
        _UIMenu = {
            ...
            Settings = {
                ...
                Audio = {
                    Library = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET",
                    UpDown = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET",
                    LeftRight = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET",
                    Select = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET",
                    Back = "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET",
...
function Menu:Close()
	...
	self.visible = false
	PlaySoundFrontend(-1, "CONTINUE", "HUD_FRONTEND_CLOTHESSHOP_SOUNDSET", 1)
	...
	local lastselect = nil
		while self.visible do
			...
			if self.title_sprite then
				...
				--drawMenuText(self.title,1,1,tx,ty - height -0.005,scale*2.4,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
				-- DrawRect(x,  y + 0.017, width, height + 0.04,cfg.sbg_color.r,cfg.sbg_color.g,cfg.sbg_color.b,cfg.sbg_color.a)
			end
			y = y + height
			ty = y - height/2.8
			tx = tx - (width/2.2)
					...
					if i >= menu.from and i <= menu.to then
						if menu.selected == i then
							...
							elseif btn.price ~= nil then	
								if btn.price == 0 then
									...
									if not btn.purchased then
										drawRightMenuText("~g~" .. btn.price .. "$",cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
									else
										drawRightMenuText("~y~Actuelle",cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
									end
								end
							...
							elseif btn.price ~= nil then
								if btn.price ~= 0 then
									if not btn.purchased then
										drawRightMenuText("~g~" .. btn.price .. "$",cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
									else
